...
export async function generatePython(document: OpenApiDocument, config: PythonConfig, output: string): Promise<number> {
  ...
  const urls = [
    ...
    config.docs?.url && `Documentation = "${config.docs.url}"`,
    config.repository && `Repository = "${config.repository}"`,
    config.repository && `Issues = "${config.repository}/issues"`,
  ].filter(Boolean);
  const projectUrls = urls.length ? `\n\n[project.urls]\n${urls.join("\n")}` : "";
  const apiName = config.homepage ? `[${config.name}](${config.homepage})` : config.name;
  const apiKey = config.apiKey.url ? `[API key](${config.apiKey.url})` : "API key";
  const docs = config.docs?.url
    ? ` The [interactive API reference](${config.docs.url}) documents every resource and includes Python examples.`
    : "";

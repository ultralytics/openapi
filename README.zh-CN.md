...
Ultralytics OpenAPI 可从声明为 OpenAPI 3.0 至 3.2.0 的规范中读取 `GET`、`POST`、`PUT`、`PATCH`、`DELETE`、`OPTIONS`、`HEAD` 和 `TRACE` 操作，并将其转换为可运行的 API 文档和类型化 SDK。将 `openapi.config.json` 指向本地文件或 URL，配置生成的软件包名称，即可从同一份契约生成两种输出。API 密钥仅保存在浏览器内存中，绝不会出现在复制的示例中。
